 ...
 *  TODO:
 *  Remove "(" nodes after linking the gap
 ...
 * ***************************************************************************/
int ExpressionParser::closeSubExprScope (bool & isChkNestedTernarys)  {
  int ret_code = GENERAL_FAILURE;
	bool isScopenerLinked = false;
	bool isOpenerParen = false;
	bool isOpenerTernary = false;

	// Init callers variable to default, expected value
	isChkNestedTernarys = false;

	// Get pointer to ExprTreeNode that contains the opening "(" placeholder
	...
	if (scopener == NULL)	{
		...
		if (scopener->originalTkn->tkn_type == SPR8R_TKN && 0 == scopener->originalTkn->_string.compare(L"("))	{
			...
			isOpenerTernary = true;

		if (isOpenerParen != isOpenerTernary)	{
			...
			if (OK == turnClosedScopeIntoTree (exprScopeStack[exprScopeStack.size() - 1]->scopedKids))	{
				...
				if (stackSize >= 1)	{
					...
					ExprTreeNodePtrVector & childList = exprScopeStack[stackSize - 1]->scopedKids;

					for (nodeR8r = childList.begin(); //exprScopeStack[stackSize - 1]->scopedKids.begin();
							nodeR8r != childList.end() && !isScopenerLinked; nodeR8r++)	{
						...
						if ((void *)currNode == (void *)scopener)	{
							if (isOpenerParen)	{
								...
								scopener->_1stChild = subExpr;
								subExpr->myParent = scopener;
							}
							isScopenerLinked = true;
							break;
						}
					}

					if (isOpenerParen && isScopenerLinked)	{
						// Need to remove that vestigial "(" from the list
						ExprTreeNodePtrVector::iterator delR8r;
						for (delR8r = childList.begin(); delR8r != childList.end(); delR8r++)	{
				...
				if (isScopenerLinked && isOpenerTernary)	{
					// With previous scope now closed, see if parent (now current) scope is also ready to be closed.
					if (subExpr->originalTkn->tkn_type == OPR8R_TKN && subExpr->originalTkn->_string == usrSrcTerms.get_ternary_2nd())	{
						// ? OPR8R 1st child (conditional before ?) hasn't been resolved yet...it's in the next scope up
						scopener->_1stChild = NULL;
						// ? OPR8R 2nd child is the TRUE path
						scopener->_2ndChild = subExpr->_1stChild;
						subExpr->_1stChild = NULL;
						// ? OPR8R 3rd child is the FALSE path
						scopener->_3rdChild = subExpr->_2ndChild;
						subExpr->_2ndChild = NULL;
						delete subExpr;
						subExpr = NULL;
						// Tell caller to immediately attempting closing the current scope
						isChkNestedTernarys = true;
						ret_code = OK;
					}
				}
				...
				if (ret_code != OK)	{
					...
					errorMsg.append (scopener->originalTkn->description());
...
int ExpressionParser::parseExpression (InterpretedFileWriter & intrprtrWriter)  {
  ...
  if (tknStream.empty())	{
  	...
  	// Consume flat stream of Tokens in current expression; attach each to an ExprTreeNode for tree transformation
		while (!isFailed && !isWholeExprClosed)	{
			if (!tknStream.empty())	{
				...
				if (!isExpectedTknType (curr_legal_tkn_types, next_legal_tkn_types, currTkn))	{
					...
					delete currTkn;
					currTkn = NULL;
					bool isNestedTernarys = false;

					if (OK != closeSubExprScope(isNestedTernarys))	{
						isFailed = true;
					} else if (isNestedTernarys)	{
						...
						while (isNestedTernarys && !isFailed)	{
		...
		if (isWholeExprClosed && !isFailed)	{
			ret_code = OK;
			exprScopeStack[0]->scopedKids[0]->showTree(exprScopeStack[0]->scopedKids[0]);
  ...
  if (ret_code != OK) {
  	if (errorMsg.length() == 0)	{
	  	...
	  	std::wcout << "Error encountered on internal source " << util.getLastSegment (util.stringToWstring(__FILE__), L"/") << ":" << errOnOurSrcLineNum << std::endl;
		}
  }
...
int ExpressionParser::turnClosedScopeIntoTree (ExprTreeNodePtrVector & currScope)  {
  ...
  // TODO: Code inspect, comment, simplify if you can
	for (outr8r = usrSrcTerms.grouped_opr8rs.begin(); outr8r != usrSrcTerms.grouped_opr8rs.end() && !isFailed && !isNowTree; outr8r++){
		...
		while (!isReachedEOL && !isFailed && !isNowTree)	{
			...
			for (currNodeR8r = currScope.begin(); currNodeR8r != currScope.end() && !isFailed && !isNowTree && !isOpr8rReady; currNodeR8r++)	{
				...
				// TODO: && !isOpr8rReady made the difference.  Was doing 6 loop iterations with a currScope size of 4 (???)
				// Possible multiple instances of current OPR8R in this expression scope.
				...
				} else if (currNode->originalTkn->tkn_type == OPR8R_TKN && currNode->_1stChild == NULL)	{
					// TODO: Bug! currNode->originalTkn is wEiRd.
					...
					for (innr8r = precedenceLvl.opr8rs.begin(); innr8r != precedenceLvl.opr8rs.end() && !isFailed && !isOpr8rReady; ++innr8r){
						...
						if (isOpr8rReady)	{
							...
							if ((tgtOpr8r.type_mask & POSTFIX) || (tgtOpr8r.type_mask & BINARY) || (tgtOpr8r.type_mask & TERNARY_2ND)
									|| (tgtOpr8r.type_mask & TERNARY_1ST && currNode->_2ndChild != NULL && currNode->_3rdChild != NULL))	{
							...
							if ((tgtOpr8r.type_mask & PREFIX) || (tgtOpr8r.type_mask & UNARY) || (tgtOpr8r.type_mask & BINARY) || (tgtOpr8r.type_mask & TERNARY_2ND))	{
							...
							// isOpr8rReady, so break loop and start over with OPR8R list @ this precedence level
							break;
						}
					}
				}
	...
	if (outr8r != usrSrcTerms.grouped_opr8rs.end() && !isFailed && isNowTree)	{
		// We made it all the way to the end of the Token stream
 ...
 * ***************************************************************************/
void ExpressionParser::printScopeStack ()	{

	std::vector<NestedScopeExpr *>::reverse_iterator scopeR8r;
	std::vector<ExprTreeNode *>::iterator kidR8r;
	int scopeLvl = exprScopeStack.size() - 1;

	std::wcout << L"********** EXPRESSION SCOPE STACK **********" << std::endl;
	for (scopeR8r = exprScopeStack.rbegin(); scopeR8r != exprScopeStack.rend(); scopeR8r++)	{
